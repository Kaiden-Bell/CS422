...
Uses a sign test where we determine the label:
...
For a test point:
    ...
    3. If:
        ...
        * > 1 class -> choose the one where pt is farthest from the boundary
            s_i / ||w_i||

`plot_data_and_boundaries(data, W, B)`

    * Plots each class with a unique color/marker
